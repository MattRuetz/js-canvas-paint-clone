...
const displayBrushSize = () => {
    brushSize.textContent =
        brushSlider.value < 10 ? `0${currentSize}` : brushSlider.value;
};
...
// Timer to revert tool label back to brush after showing a temp action.
const backToBrushSetTimeout = (time_ms) => {
    setTimeout(switchToBrush, time_ms);
};

// Create Canvas
const createCanvas = () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight - 50; //50px at top to leave room for toolbar
...
clearCanvasBtn.addEventListener('click', () => {
    ...
    backToBrushSetTimeout(BRUSH_TIME);
});
